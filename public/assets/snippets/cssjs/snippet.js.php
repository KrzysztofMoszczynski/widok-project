...
if ($minify == '1') {
  ...
  file_put_contents(MODX_BASE_PATH.$folder.'scripts.min.'.substr(md5(max($v)),0,20).'.js', $minified_javascript);
  //return '<script src="'.$modx->config['site_url'].$folder.'scripts.min.'.substr(md5(max($v)),0,20).'.js"></script>';
  return $modx->regClientScript($folder.'scripts.min.'.substr(md5(max($v)),0,20).'.js');
}
else {
  $links = '';
  foreach ($filesArr as $key => $value) {
    //$links .= '<script src="'.$modx->config['site_url'].trim($value).'?v='.substr(md5($v[$key]),0,3).'"></script>';	
    $links .= $modx->regClientScript(trim($value).'?v='.substr(md5($v[$key]),0,3));
  }	
  return $links;
}
?>