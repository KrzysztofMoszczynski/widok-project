...
if ($minify == '1') {
  ...
  $min = $minified->merge( MODX_BASE_PATH.$folder.'styles.min.'.substr(md5(max($v)),0,20).'.css', 'css', $filesForMin );
  //return '<link rel="stylesheet" href="'.$modx->config['site_url'].$folder.'styles.min.'.substr(md5(max($v)),0,20).'.css" />';
  return $modx->regClientCSS($folder.'styles.min.'.substr(md5(max($v)),0,20).'.css');
}else{
  $links = '';
  foreach ($filesArr as $key => $value) {
    //$links .= '<link rel="stylesheet" href="'.$modx->config['site_url'].trim($value).'?v='.substr(md5($v[$key]),0,3).'" />';
    $links .= $modx->regClientCSS(trim($value).'?v='.substr(md5($v[$key]),0,3));	
  }	
  return $links;
}
?>
